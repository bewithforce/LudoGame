...
class Clobrdo(object):
    ...
    def player_move(self, player):
        ...
        if self.moved > 1:
            raise MovedMoreThanOnce("Player moved more than once", player, self)
    ...
    def positions(self, player, field_from):
        ...
        from_board = field_from < self.board_size and self.board.get(absolute_from) == player
        from_finish = field_from >= self.board_size and player.finish[field_from - self.board_size]

        field_to = field_from + self.die
        absolute_to = player.unshift_field(field_to)
        finish_border = self.board_size + self.n_pieces
        to_finish = self.board_size <= field_to < finish_border and not player.finish[field_to - self.board_size]
        to_board = field_to < self.board_size and self.board.get(absolute_to) != player

        return absolute_from, from_board, from_finish, absolute_to, to_board, to_finish
    ...
    def move_possible(self, player, field_from):
        ...
        if field_from == START and self.die == self.n_die:
            if len(player.pieces()) < self.n_pieces and self.board.get(player.start) != player:
                return True

        _, from_board, from_finish, _, to_board, to_finish = self.positions(player, field_from)
    ...
    def move(self, player, field_from):
        ...
        if self.move_possible(player, field_from):
            abs_from, from_board, from_finish, abs_to, to_board, to_finish = self.positions(player, field_from)
